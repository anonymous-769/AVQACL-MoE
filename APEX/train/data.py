...
from APEX.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from torch.utils.data import Dataset

from APEX import conversation as conversation_lib
from APEX.model import *
from APEX.mm_utils import tokenizer_image_token,tokenizer_image_audio_token,tokenizer_image_audio_video_token
...
class DataArguments:
    ...
    pad_audio: bool = True
    mix_va: bool = False
    audio_processor: Optional[Any] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # 锚点MoE相关参数
    moe_training_mode: Optional[str] = field(default=None, metadata={"help": "MoE训练模式: 'anchor' 表示锚点模式"})
    cur_task: Optional[int] = field(default=None, metadata={"help": "当前训练的任务ID"})
    ...
    num_experts_per_tok: Optional[int] = field(default=1, metadata={"help": "每个token使用的专家数量"})
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'video' in sources[0]:
            ...
            for frame_file in self.list_data_dict[i]['video']:
                image_processor = self.data_args.image_processor
                # 检查是否为绝对路径或包含 AUDIOSTART
                if AUDIOSTART in frame_file or os.path.isabs(frame_file):
                    frame = Image.open(frame_file).convert('RGB')
                else:
                    # 使用相对路径，需要从 zip 文件中提取
                    frame_folder = self.data_args.video_folder if self.data_args.video_folder is not None else self.data_args.image_folder
                    if frame_folder is None:
                        raise ValueError(f"video_folder and image_folder are both None, cannot process relative path: {frame_file}")
