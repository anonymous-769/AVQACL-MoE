#!/bin/bash
#SBATCH -J Test
#SBATCH -o logs/train_task_%A_%a.log
#SBATCH -e logs/train_task_%A_%a.err
...
echo "================================================================================"
echo "Logging to: logs/train_task_${SLURM_ARRAY_JOB_ID}_${TASK_ID}.log"
echo "Error log: logs/train_task_${SLURM_ARRAY_JOB_ID}_${TASK_ID}.err"
...
train_task() {
    ...
    # 为每个任务使用不同的端口号（避免端口冲突）
    local MASTER_PORT=$((9873 + $task_id))
    export MASTER_PORT
...
case $TASK_ID in
    0)
        ...
        if [ $? -ne 0 ]; then
            echo "Error: Task 0 (Come_From) training failed!" >&2
            exit 1
        fi
        echo "Task 0 (Come_From) training completed successfully."
        ;;
    1)
        train_task 1 "data_sample/train_Happening.json" "output/AVQA/AVQACL-MoE_task1_ckpt"
        if [ $? -ne 0 ]; then
            echo "Error: Task 1 (Happening) training failed!" >&2
            exit 1
        fi
    ...
    2)
        ...
        if [ $? -ne 0 ]; then
            echo "Error: Task 2 (Where) training failed!" >&2
            exit 1
        fi
    ...
    3)
        ...
        if [ $? -ne 0 ]; then
            echo "Error: Task 3 (Which) training failed!" >&2
            exit 1
        fi
...
echo "================================================================================"
echo "Task $TASK_ID training completed!"
echo "Output files:"
echo "  - Expert weight: output/experts/expert_task_${TASK_ID}.pt"
echo "  - Anchor: output/anchors/anchor_task_${TASK_ID}.pt"
echo "================================================================================"