...
from AVQACL_MoE.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, AUDIO_TOKEN_INDEX, VIDEO_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, 'mm_projector', None) is None:
            ...
            # In case it is frozen by LoRA, only set requires_grad if tune_mm_mlp_adapter is True
    ...
    def initialize_audio_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, 'mm_audio_aligner', None) is None:
            ...
            # In case it is frozen by LoRA, only set requires_grad if tune_mm_audio_projector is True
            if getattr(model_args, 'tune_mm_audio_projector', False):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_audio_and_vision(
        ...
        if (vision_tower is None and audio_tower is None) or (images is None and  input_features is None and videos is None) or (input_ids.shape[1] == 1 and input_ids[0][0] == 1):
            ...
            # 早期返回时，没有inputs_embeds，所以返回原始input_ids
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            if attention_mask is not None:
                ...
                for cur_attention_mask, cur_new_labels, cur_new_labels_align in zip(attention_mask, _new_labels, new_labels):
                    # 计算左侧填充长度（多模态特征增加的长度）
                    left_pad_length = cur_new_labels.shape[0] - labels.shape[1]
                    # 计算右侧填充长度（对齐到最大长度）
                    right_pad_length = cur_new_labels_align.shape[0] - cur_new_labels.shape[0]
                    
                    # 确保填充长度不为负数
                    left_pad_length = max(0, left_pad_length)
                    right_pad_length = max(0, right_pad_length)
                    
                    new_attn_mask_pad_left = torch.full((left_pad_length,), True, dtype=attention_mask.dtype, device=attention_mask.device)
                    new_attn_mask_pad_right = torch.full((right_pad_length,), False, dtype=attention_mask.dtype, device=attention_mask.device)
                    cur_new_attention_mask = torch.cat((new_attn_mask_pad_left, cur_attention_mask, new_attn_mask_pad_right), dim=0)
                    new_attention_mask.append(cur_new_attention_mask)
                attention_mask = torch.stack(new_attention_mask, dim=0)
                
                # 确保attention_mask与new_labels形状匹配
                if attention_mask.shape != new_labels.shape:
                    batch_size, seq_len = new_labels.shape
                    attention_mask = torch.ones((batch_size, seq_len), dtype=torch.bool, device=attention_mask.device)
            ...
            if attention_mask is not None:
                # 计算需要填充的长度
                pad_length = new_input_embeds.shape[1] - attention_mask.shape[1]
                if pad_length > 0:
                    # 在左侧填充True（表示这些位置需要注意）
                    new_attn_mask_pad_left = torch.full((attention_mask.shape[0], pad_length), True, dtype=attention_mask.dtype, device=attention_mask.device)
                    attention_mask = torch.cat((new_attn_mask_pad_left, attention_mask), dim=1)
                elif pad_length < 0:
                    # 如果attention_mask比new_input_embeds长，截断attention_mask
                    attention_mask = attention_mask[:, :new_input_embeds.shape[1]]
                
                # 确保attention_mask的形状与new_input_embeds匹配
                if attention_mask.shape != new_input_embeds.shape[:2]:
                    # 如果仍然不匹配，重新创建attention_mask
                    batch_size, seq_len = new_input_embeds.shape[:2]
                    attention_mask = torch.ones((batch_size, seq_len), dtype=torch.bool, device=attention_mask.device)
                
                assert attention_mask.shape == new_input_embeds.shape[:2]
        # 当new_input_embeds不为None时，应该返回None作为input_ids，避免同时传递两者
        final_input_ids = None if new_input_embeds is not None else input_ids
        return final_input_ids, attention_mask, past_key_values, new_input_embeds, new_labels
